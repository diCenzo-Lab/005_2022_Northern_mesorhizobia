...
# Rhizobium genomes
$genomes = 'Input_lists/genomeList_rhizo.txt'; # file to get the information of the files
...
foreach $species (@species) {
		$protein = $species;
		$species .= '.faa';
		$species2 = 'Genome_files/Rhizobium_reannotated/';
		$species2 .= $species;
		$output = 'Rooted_phylogeny/MLSA_phylogeny/Proteomes/';
		$output .= $species;
		open($in, '<', $species2);
		open($out, '>', $output);
		while(<$in>) {
